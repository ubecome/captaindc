...
module.exports = (server) =>
{
	...
	io.on('connection', (socket) =>
	{
		...
		if(user)
		{
			...
			{
				callWhenRobotStateAvailable(() => socket.emit('robot-battery-level', robotState.battery.percent));
			});

			socket.on('robot-docking-state-request', () =>
			{
				callWhenRobotStateAvailable(() => socket.emit('robot-docking-state', robotState.sensors['/sensors/docking_status']));
			});

			socket.on('robot-position-request', () =>
			{
				callWhenRobotStateAvailable(() => socket.emit('robot-position', robotState.position));
			});

			socket.on('robot-state-request', () =>
			{
				callWhenRobotStateAvailable(() => socket.emit('robot-state', robotState.robot_state.state));
			});

			socket.on('robot-emergency-button-request', () =>
			{
				callWhenRobotStateAvailable(() => socket.emit('robot-emergency-button', robotState.sensors.emergency_button));
			});
			...
			socket.on('pilot-request', () =>
			{
				...
				if(granted)
				{
					...
					socket.broadcast.to(homeRoom).emit('pilot-changed', {name: user.name});
				}
			});
	...
	startRobotStatePolling = () =>
	{
		...
		{
			robotStatePollingDaemon = new Daemon('robot-state-polling', 0, 500, null, () =>
			{
				RobotLink.api({url: '/state'}, (err, response) =>
				{
					if(!err)
					{
						...
						if(newRobotState.result && newRobotState.result === 'Unauthorized')
						{
						...
						newRobotState.position.theta = parseFloat(parseFloat(newRobotState.position.theta).toFixed(1));
						newRobotState.battery.percent = Math.round(newRobotState.battery.percent);

						newRobotState.sensors['/sensors/docking_status'] = newRobotState.sensors['/sensors/docking_status'] !== '0';

						if(robotState)
						{
							if(newRobotState.battery.percent !== robotState.battery.percent)
							{
								io.to(homeRoom).emit('robot-battery-level', newRobotState.battery.percent);
							}

							if(newRobotState.robot_state.state !== robotState.robot_state.state)
							{
								io.to(homeRoom).emit('robot-state', newRobotState.robot_state.state);
							}

							if(newRobotState.sensors['/sensors/docking_status'] !== robotState.sensors['/sensors/docking_status'])
							{
								io.to(homeRoom).emit('robot-docking-state', newRobotState.sensors['/sensors/docking_status']);
							}

							if(newRobotState.position.x !== robotState.position.x || newRobotState.position.y !== robotState.position.y || newRobotState.position.theta !== robotState.position.theta)
							{
								io.to(homeRoom).emit('robot-position', newRobotState.position);
							}

							if(newRobotState.sensors.emergency_button !== robotState.sensors.emergency_button)
							{
								io.to(homeRoom).emit('robot-emergency-button', newRobotState.sensors.emergency_button);
