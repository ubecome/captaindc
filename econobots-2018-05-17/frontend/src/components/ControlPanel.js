...
class ControlPanel extends AbstractComponent
{
	...
	onDrag(x, y)
	{
		...
		if(x !== 0 || y !== 0)
		{
			if(!this.daemon)
			{
				this.daemon = new Daemon('sending-joytick-values', 0, 500, undefined, () =>
				{
					ajax('POST', 'robot/move', {x: this.joystick.y, y: -this.joystick.x});
				});
			}
		}
		else if(this.daemon)
		...
		return (
			<div id='control-panel-container' className='horizontal'>
				...
				<div className='control-widget horizontal' hidden={hidden}>
					<ControlPanelButton size={[55, 90]} cornerRadius={{left: 40}} icon='undo' disabled={disabled} onClick={() => this.turnBy(-2 * Math.PI)}/>
					<Joystick onDrag={this.onDrag} disabled={disabled}/>
					<ControlPanelButton size={[55, 90]} cornerRadius={{right: 40}} icon='repeat' disabled={disabled} onClick={() => this.turnBy(2 * Math.PI)}/>
				</div>
				<div className='control-widget horizontal' hidden={hidden}>
					<ControlPanelButton id="light-button" size={[80, 45]} cornerRadius={{left: 30}} icon='lightbulb' disabled={disabled} active={this.state.lightOn} onClick={this.toggleLight}/>
					<ControlPanelButton id='alarm-button' size={[80, 45]} icon='alarm' disabled={disabled} active={this.state.alarmOn} onClick={this.toggleAlarm}/>
					<ControlPanelButton id='go-to-dock-button' size={[80, 45]} cornerRadius={{right: 30}} icon='plug' disabled={disabled} onClick={this.gotoDock}/>
					{/*
					<ControlPanelButton id="log-button" size={[80, 45]} cornerRadius={{topRight: 30}} icon='write' disabled={disabled} onClick={() => {this.setState({displayLogModal: true})}}/>
					<Modal basic size='small' open={this.state.displayLogModal} onClose={this.closeLogModal} >
						<Modal.Content>
							<Input type='text' fluid placeholder={i18n.get('event-log.message-placeholder')} onChange={(e) => {this.logValue = e.target.value}} action>
								<input onKeyPress={(e) => {if (e.key === 'Enter') this.submitLog()}} autoFocus/>
								<Button type='submit' onClick={this.submitLog}>{i18n.get('submit')}</Button>
							</Input>
						</Modal.Content>
					</Modal>
					*/}
				...
				<div className='control-widget control-widget-right'>
					<BatteryIndicator level={this.state.batteryLevel} charging={this.state.docked} robotState={this.state.robotState}/>
				</div>
			</div>
		);
	}
